	...
	<form-view @submitted="update('additional-tasks.update', additionalTask.id)" title="Update Additional Task" :breadcrumb="breadcrumb">
		<template #form>
			<form-group class="border-b gap-8">
				...
				<!-- Due By -->
				<div class="col w-full">
					<jet-label class="md:w-1/4" for="dueDate" value="Due Date" />
					<div class="w-full mt-1">
						<jet-input v-model="form.dueDate" id="dueDate" type="date" max="30" class="w-full" autocomplete="dueDate" />
			...
			<form-group class="border-b gap-8">
				...
				<div class="col w-full">
					...
					<div class="w-full mt-1">
						<jet-select v-model="form.status" id="status" class="w-full" :options="statusOptions" autocomplete="status" required :key="componentKey" track="value" />
...
export default {
	...
	mounted() {
		Object.assign(this.form, this.additionalTask);
		this.componentKey++
	},
	methods: {
		//
